...
fname='sinayGravExp('+str(round(eta,3))+')_p'+str(round(perturbation,3))+'_grid' + str(grid)+'_maxIte'+str(N)+'_constPhi' + str(theta)
fig, ax = plt.subplots()
ax.set_aspect('equal')

phiAx = np.linspace(0,np.pi/2-0.2,grid) #start and end needs to be same as in geXYAng() steps needs to be +1
startAngAx=np.linspace(0,np.pi/2-0.2,grid)
...
for r in rAx:
    xyang=getXYAng(r,epsilon,grid,theta)
...
ax.pcolormesh(startAngAx, rAx, nColAx, shading='nearest', vmin=nColAx.min(), vmax=nColAx.max())
